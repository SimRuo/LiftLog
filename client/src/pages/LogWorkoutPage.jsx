...
import { Typography, Box, Button, TextField, Card, CardContent, Stack, Alert, CircularProgress, Chip, IconButton } from "@mui/material";
import { SaveRounded, HotelRounded, DeleteRounded, AddRounded } from "@mui/icons-material";
...
export default function LogWorkoutPage() {
  ...
  const saveDraft = useCallback((draftDate, draftNotes, draftExercises, day) => {
    try {
  ...
  useEffect(() => {
    workoutsApi
      ...
      .then((data) => {
        ...
        // Try to restore draft if it matches this plan day
        const saved = localStorage.getItem(DRAFT_KEY);
        if (saved) {
          try {
            const draft = JSON.parse(saved);
            if (draft.planDayId === data.planDayId) {
              setDate(draft.date);
  ...
  useEffect(() => {
    if (!nextDay || exercises.length === 0) return;
    saveDraft(date, notes, exercises, nextDay);
  }, [date, notes, exercises, nextDay, saveDraft]);
            ...
            {ex.sets.map((set, setIdx) => (
              <Stack key={setIdx} direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
                ...
                <TextField
                  ...
                  inputProps={{ min: 0 }}
                />
                <IconButton size="small" onClick={() => removeSet(exIdx, setIdx)} disabled={ex.sets.length <= 1}>
